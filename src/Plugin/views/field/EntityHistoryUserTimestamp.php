...
namespace Drupal\entity_history\Plugin\views\field;
 ...
 *
 * @ViewsField("entity_history_user_timestamp")
 */
class EntityHistoryUserTimestamp extends Node {
  ...
  public function render(ResultRow $values) {
    ...
    if (\Drupal::currentUser()->isAuthenticated()) {
      ...
      if (!$last_read && $changed > ENTITY_HISTORY_READ_LIMIT) {
        $mark = MARK_NEW;
      }
      elseif ($changed > $last_read && $changed > ENTITY_HISTORY_READ_LIMIT) {
        $mark = MARK_UPDATED;
      }
      elseif ($last_comment > $last_read && $last_comment > ENTITY_HISTORY_READ_LIMIT) {
        $mark = MARK_UPDATED;
      }
      $build = array(
        '#theme' => 'mark',
